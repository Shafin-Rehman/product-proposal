...
describe('budgetHealth specification', () => {
  describe('buildOverallBudgetHealth', () => {
    ...
    it('treats category total as the cap and surfaces near limit when remaining budget is tight', () => {
      expect(buildOverallBudgetHealth({
        summary: {
          month: '2026-03-01',
          monthly_limit: null,
  ...
  describe('buildCategoryBudgetHealth', () => {
    ...
    it('returns no-budget when actuals are unavailable and no limit is set', () => {
      expect(buildCategoryBudgetHealth({
        monthlyLimit: null,
        spent: null,
        actualsAvailable: false,
  ...
  describe('getMonthProgressState', () => {
    ...
    it('falls back to the current clock when referenceDate is not parseable', () => {
      ...
      expect(state.monthLength).toBe(31)
      expect(state.activeDay).toBeGreaterThan(0)
    })
  })
})
