...
describe('monthlyReportExport specification', () => {
  beforeEach(() => {
    db.query.mockReset()
  ...
  describe('monthly report CSV helpers', () => {
    ...
    it('breaks same-day ties with created_at first, then stable type ordering', () => {
      const csv = buildMonthlyReportCsv({
        month: '2026-03-01',
        summary: { total_income: '40.00', total_expenses: '25.00' },
        transactions: [
          {
            id: 'income-row',
            month: '2026-03-01',
  ...
  describe('monthly report data access', () => {
    it('returns both expense and income activity for the chosen export month', async () => {
      db.query
      ...
      expect(db.query.mock.calls[0][1]).toEqual(['uid', '2026-03-01', '2026-04-01'])
      expect(db.query.mock.calls[1][1]).toEqual(['uid', '2026-03-01', '2026-04-01'])
      expect(rows).toHaveLength(2)
      expect(rows).toEqual(expect.arrayContaining([
        expect.objectContaining({ id: 'exp-1', type: 'expense', amount: '12.00', date: '2026-03-05' }),
        expect.objectContaining({ id: 'inc-1', type: 'income', amount: '200.00', date: '2026-03-06' }),
      ]))
    })
