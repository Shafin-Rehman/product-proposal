...
describe('planner specification', () => {
  describe('buildPlannerRows', () => {
    it('builds an empty editable row when a category has no spend and no plan', () => {
      ...
      const rows = buildPlannerRows(input)
    ...
    it('builds an active planned row that is still on track', () => {
      ...
      const [row] = buildPlannerRows(input)
  ...
  describe('planner monthly health semantics', () => {
    it('surfaces near-limit overall health from category plan totals when no single monthly cap exists', () => {
      const snapshot = {
        summary: {
          month: '2026-03-01',
          monthly_limit: null,
      ...
      const health = buildOverallBudgetHealth(snapshot)
  ...
  describe('planner amount draft validation', () => {
    ...
    it.each([
      ['-5',   {},                  { isValid: false, kind: 'negative',        message: 'Amount cannot be negative.' }],
      ['abc',  {},                  { isValid: false, kind: 'malformed',       message: 'Enter a valid dollar amount.' }],
      ['1.005',{},                  { isValid: false, kind: 'too_many_decimals', message: 'Enter a dollar amount with no more than 2 decimal places.' }],
      ['0',    {},                  { isValid: false, kind: 'zero',            message: 'Enter an amount greater than $0.' }],
      ['0',    { hasSavedPlan: true }, { isValid: false, kind: 'zero',         message: 'Use Clear plan to return this category to Not set.' }],
      ['',     {},                  { isValid: false, kind: 'empty',           message: '' }],
    ])('rejects invalid draft %j with kind %s', (draft, opts, expected) => {
      expect(getPlannerAmountDraftValidation(draft, opts)).toEqual(expected)
    })
  })
